...
==== 新增功能
* ✅ *自动重连机制*: 指数退避策略，最多 5 次重试，最大延迟 30 秒
...
==== 新增文档
* ✅ `docs/dashboard/mqtt-guide.adoc` - MQTT 使用指南
** 系统架构图
** Topic 规范说明
** 配置说明
** 上位机和 ESP32 代码示例
...
==== 更新文档
* ✅ `dashboard/README.adoc` - 添加 MQTT 功能说明和依赖安装指南
* ✅ `README.adoc` - 更新项目结构说明
...
. 创建连接：`broker.emqx.io:1883`
. 订阅 `heterolink/subboard/#`
. 观察测试程序发布的消息
...
== 技术亮点
. *自动重连*: 指数退避算法，避免频繁重连冲击 Broker
. *Last Will*: 设备异常断开时自动发布离线消息
. *信号槽解耦*: MqttChannel 与业务逻辑完全解耦
. *条件编译*: 支持有/无 Qt MQTT 模块两种编译模式
. *详细日志*: 所有关键操作都有日志输出，便于调试

== 参考资料
* https://doc.qt.io/qt-6/qtmqtt-index.html[Qt MQTT 文档]
